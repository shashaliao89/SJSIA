    ...
    {loading ? <EmptyState message="載入中…" /> : kols.length === 0 ? <EmptyState message="目前尚無公開 KOL 資料" /> : <>
      ...
      {filtered.length === 0 ? <EmptyState message="沒有符合目前篩選條件的 KOL" /> : <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{filtered.map(kol => <Card key={kol.id} className="flex h-full flex-col overflow-hidden p-5">
        <div className="flex items-center gap-4">
          <div className="relative flex h-[72px] w-[72px] shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/15 bg-gradient-to-br from-white/10 to-black text-lg font-black text-white/35 shadow-[0_0_24px_rgba(255,255,255,0.05)]">
            <span>{initials(kol.name)}</span>
            {kol.avatar_url ? <img src={kol.avatar_url} alt={`${kol.name} 大頭照`} width={72} height={72} className="absolute inset-0 h-full w-full object-cover" loading="lazy" onError={event => { event.currentTarget.style.display="none"; }} /> : null}
          </div>
          <div className="min-w-0"><h3 className="line-clamp-2 text-lg font-black leading-snug text-white">{kol.name}</h3><p className="mt-1 text-sm font-bold text-[#CFFF1A]">{formatFollowers(kol.follower_count)} 粉絲</p></div>
        </div>
        <div className="mt-5 flex flex-1 flex-col border-t border-white/10 pt-4">{kol.content_types?.length ? <div className="flex flex-wrap gap-2">{kol.content_types.slice(0,5).map(tag => <Badge key={tag}>{tag}</Badge>)}</div> : null}<div className="mt-4 flex-1">{kol.ig_url ? <a href={kol.ig_url} target="_blank" rel="noreferrer" className="text-sm font-bold text-[#CFFF1A] hover:underline">查看 Instagram ↗</a> : <span className="text-sm text-gray-600">Instagram 待補</span>}</div><Button className="mt-5 w-full" disabled={!kol.open_to_contact} variant={kol.contacted ? "secondary" : "primary"} onClick={() => openConversation(kol)}>{kol.contacted ? "已聯繫過" : "聯繫／洽談申請"}</Button></div>
      </Card>)}</div>}
    </>}
